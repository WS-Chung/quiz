...
/* ══════════════════════════════════════
   image_select 테두리 색상 변환
   라이브러리가 그리는 빨간 테두리를
   CSS hue-rotate 로 보라색 계열로 변환
   굵기는 라이브러리 기본값(약 3px) 사용
   ══════════════════════════════════════ */
[data-testid="stIFrame"] iframe {
    border: none !important;
    /* 빨강(0deg) → 보라(270deg) 로 색상 회전 */
    filter: hue-rotate(270deg) saturate(1.4) brightness(0.95);
}

/* ══════════════════════════════════════
   텍스트 선택지 공통 shape
   secondary(미선택) · primary(선택됨) 동일
   ══════════════════════════════════════ */
...
button[data-testid="stBaseButton-primary"] {
    ...
    border-radius: 16px !important;   /* 두 타입 동일한 모서리 */
...
/* 확인 / 다시하기 버튼만 pill 모양 + 더 큰 높이로 덮어씀
   — key 로 구분 불가하므로 aria-label 활용 */
...
if not st.session_state.complete:
    ...
    # ══════════════════════════════════════
    # 텍스트 퀴즈
    # 선택됨 → primary  (보라 채움, 흰 텍스트)
    # 미선택 → secondary (흰 배경, 보라 테두리)
    # 두 타입 동일한 height·radius·font
    # ══════════════════════════════════════
    else:
